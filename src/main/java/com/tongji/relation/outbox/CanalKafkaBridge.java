...
public class CanalKafkaBridge implements SmartLifecycle {
                ...
                while (running) {
                    ...
                    for (CanalEntry.Entry entry : message.getEntries()) {
                        ...
                        try {
                            ...
                            // 提取 fromUserId 作为 Partition Key，保证同一用户的操作顺序
                            String partitionKey = extractPartitionKey(msgNode);
                            if (partitionKey != null) {
                                kafka.send(OutboxTopics.CANAL_OUTBOX, partitionKey, json);
                            } else {
    ...
    /**
     * 从消息中提取 Partition Key（fromUserId）。
     * 优先使用 fromUserId 保证同一用户的操作顺序。
     * @param msgNode 消息节点
     * @return Partition Key，如果无法提取则返回 null
     */
    private String extractPartitionKey(ObjectNode msgNode) {
        try {
            ArrayNode dataArray = (ArrayNode) msgNode.get("data");
            if (dataArray != null && !dataArray.isEmpty()) {
                ObjectNode rowNode = (ObjectNode) dataArray.get(0);
                String payload = rowNode.get("payload").asText();
                JsonNode payloadNode = objectMapper.readTree(payload);

                // 优先使用 fromUserId
                JsonNode fromUserNode = payloadNode.get("fromUserId");
                if (fromUserNode != null && !fromUserNode.isNull()) {
                    return fromUserNode.asText();
                }

                // 其次使用 userId（计数事件）
                JsonNode userIdNode = payloadNode.get("userId");
                if (userIdNode != null && !userIdNode.isNull()) {
                    return userIdNode.asText();
                }
            }
        } catch (Exception e) {
            log.debug("Failed to extract partition key", e);
        }
        return null;
    }
}