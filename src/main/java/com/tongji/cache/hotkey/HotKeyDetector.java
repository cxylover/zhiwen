...
public class HotKeyDetector {
    ...
    public void record(String key) {
        int[] arr = counters.computeIfAbsent(key, k -> new int[segments]);//key 不存在时创建长度为 6 的数组
        arr[current.get()]++;//在当前活跃分段上 +1
    }
     ...
     * 触发频率由配置 `cache.hotkey.segment-seconds` 指定（单位秒）。每 10 秒执行一次
     */
