...
public class VerificationService {
    ...
    public SendCodeResult sendCode(VerificationScene scene, String identifier) {
        ...
        AuthProperties.Verification cfg = properties.getVerification();// 获取配置
        enforceSendInterval(scene, identifier, cfg.getSendInterval());// 发送间隔限制
        enforceDailyLimit(scene, identifier, cfg.getDailyLimit()); // 日次数限制

        String code = generateNumericCode(cfg.getCodeLength());// 生成随机码
        codeStore.saveCode(scene.name(), identifier, code, cfg.getTtl(), cfg.getMaxAttempts());// 保存到存储
        codeSender.sendCode(scene, identifier, code, (int) cfg.getTtl().toMinutes());// 调用发送器
        return new SendCodeResult(identifier, scene, (int) cfg.getTtl().toSeconds());// 返回结果
    }
