...
public class AuthService {
    ...
    public SendCodeResponse sendCode(SendCodeRequest request) {
        ...
        SendCodeResult result = verificationService.sendCode(request.scene(), normalized);// 发送验证码
    ...
    public AuthResponse register(RegisterRequest request, ClientInfo clientInfo) {
        if (!request.agreeTerms()) {// 未同意协议
            throw new BusinessException(ErrorCode.TERMS_NOT_ACCEPTED);
        }
        validateIdentifier(request.identifierType(), request.identifier());// 验证标识格式
        String identifier = normalizeIdentifier(request.identifierType(), request.identifier());// 格式化标识
        if (identifierExists(request.identifierType(), identifier)) {  // 已存在
            throw new BusinessException(ErrorCode.IDENTIFIER_EXISTS);// 抛出冲突异常
        }
        ensureVerificationSuccess(verificationService.verify(VerificationScene.REGISTER, identifier, request.code()));// 验证验证码

        User user = User.builder()// 创建用户
        ...
        if (StringUtils.hasText(request.password())) {// 设置密码
            validatePassword(request.password());
            user.setPasswordHash(passwordEncoder.encode(request.password().trim()));// 密码加密
        }

        userService.createUser(user);
        TokenPair tokenPair = jwtService.issueTokenPair(user);
        storeRefreshToken(user.getId(), tokenPair);// 保存刷新令牌
        loginLogService.record(user.getId(), identifier, "REGISTER", clientInfo.ip(), clientInfo.userAgent(), "SUCCESS");// 记录登录审计
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        validateIdentifier(request.identifierType(), request.identifier());// 验证标识格式
        String identifier = normalizeIdentifier(request.identifierType(), request.identifier());// 标准化标识
        Optional<User> userOptional = findUserByIdentifier(request.identifierType(), identifier);// 查找用户
        if (userOptional.isEmpty()) {
            throw new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND);// 用户不存在
        }
        User user = userOptional.get();// 用户存在
        String channel;
        if (StringUtils.hasText(request.password())) {// 密码
            channel = "PASSWORD";
            if (!StringUtils.hasText(user.getPasswordHash()) || !passwordEncoder.matches(request.password(), user.getPasswordHash())) {// 密码错误
                loginLogService.record(user.getId(), identifier, channel, clientInfo.ip(), clientInfo.userAgent(), "FAILED");
                throw new BusinessException(ErrorCode.INVALID_CREDENTIALS);
            }
        } else if (StringUtils.hasText(request.code())) {// 验证码
            channel = "CODE";
        ...
        TokenPair tokenPair = jwtService.issueTokenPair(user);// 签发令牌对
        storeRefreshToken(user.getId(), tokenPair);// 保存刷新令牌白名单
        loginLogService.record(user.getId(), identifier, channel, clientInfo.ip(), clientInfo.userAgent(), "SUCCESS");// 登录审计
        return new AuthResponse(mapUser(user), mapToken(tokenPair));// 返回响应
    }
    ...
    public TokenResponse refresh(TokenRefreshRequest request) {
        Jwt jwt = decodeRefreshToken(request.refreshToken());// 解析刷新令牌

        if (!Objects.equals("refresh", jwtService.extractTokenType(jwt))) {// 刷新令牌类型错误
            throw new BusinessException(ErrorCode.REFRESH_TOKEN_INVALID);
        }

        long userId = jwtService.extractUserId(jwt);// 从令牌中提取用户 ID
        String tokenId = jwtService.extractTokenId(jwt);// 从令牌中提取令牌 ID

        if (!refreshTokenStore.isTokenValid(userId, tokenId)) {// 刷新令牌无效
            throw new BusinessException(ErrorCode.REFRESH_TOKEN_INVALID);
        }

        User user = findUserById(userId).orElseThrow(() -> new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND));// 查找用户
        TokenPair tokenPair = jwtService.issueTokenPair(user);// 签发令牌对
        refreshTokenStore.revokeToken(userId, tokenId);// 撤销旧刷新令牌
        storeRefreshToken(userId, tokenPair);// 保存新刷新令牌白名单

        return mapToken(tokenPair);// 返回令牌响应
    }
    ...
    public void logout(String refreshToken) {
        decodeRefreshTokenSafely(refreshToken).ifPresent(jwt -> {// 解析刷新令牌
